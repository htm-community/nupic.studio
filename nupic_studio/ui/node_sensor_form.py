...
class SensorForm(QtWidgets.QDialog):
    ...
    def setControlsValues(self):
        ...
        node = Global.architecture_form.design_panel.under_mouse_node
    ...
    def buttonNewEncoding_click(self, event):
        ...
        encoding_form.encoding_idx = -1
        encoding_form.encodings = self.encodings
        dialog_result = encoding_form.exec_()
    ...
    def buttonEditEncoding_click(self, event):
        encoding_form = EncodingForm()
        encoding_form.encoding_idx = self.list_box_encodings.currentRow()
        encoding_form.encodings = self.encodings
        encoding_form.setControlsValues()
        dialog_result = encoding_form.exec_()
        if dialog_result == QtWidgets.QDialog.Accepted:
            self.encodings_changed = True
            self.updateEncodingsListBox()
